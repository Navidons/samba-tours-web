...
export default function OptimizedGalleryImage({ 
  ...
  const [isInView, setIsInView] = useState(true) // Always show cards immediately
  ...
  useEffect(() => {
    if (!priority && item.src && typeof window !== 'undefined') {
  ...
  return (
    <Card
      ref={imageRef}
      className={`group overflow-hidden hover:shadow-lg md:hover:shadow-xl transition-all duration-200 cursor-pointer border-emerald-100 hover:border-emerald-200 ${
    ...
    >
      <div className="relative overflow-hidden">
        ...
        >
          ...
          {/* Actual image - always renders for real-time loading */}
          {!hasError && item.src && (
            <Image
              src={item.src}
              alt={item.alt}
              ...
              quality={viewMode === "masonry" ? 75 : 80}
              priority={priority}
              loading="eager"
              placeholder="blur"
