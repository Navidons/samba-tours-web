...
export default function GalleryGrid({ images = galleryItems, viewMode = "masonry" }: GalleryGridProps) {
  ...
  const renderImageCard = (item: typeof galleryItems[0], index: number) => (
    <Card
      key={item.id}
      className={`group overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer border-orange-100 hover:border-orange-200 ${
        viewMode === "masonry" ? "break-inside-avoid" : ""
      }`}
      onClick={() => setSelectedImage(index)}
    >
      <div className="relative overflow-hidden">
        <div className={`relative ${viewMode === "grid" ? "aspect-[4/3]" : getGridItemClass(item.aspectRatio)} w-full`}>
          <Image
          ...
          {/* Loading skeleton */}
          {!loadedImages.has(item.id) && <div className="absolute inset-0 bg-orange-100 animate-pulse" />}
