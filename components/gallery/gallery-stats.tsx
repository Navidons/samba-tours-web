...
export default function GalleryStats({ totalImages, totalVideos: propTotalVideos, categories, locations }: GalleryStatsProps) {
  ...
  return (
    <section className="section-padding bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl border border-orange-100">
      <div className="container-max">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-12">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="text-center p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border border-orange-100 hover:border-orange-200"
            >
              ...
              <div className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-red-600 mb-1">
                {stat.value}
              </div>
                    ...
                    return (
                      <div key={category.id} className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          />
                          <span className="font-medium text-earth-800">{category.name}</span>
                        </div>
                        <span className="text-sm text-earth-600">{totalCount} items</span>
                      </div>
                    )
                  })}
                </div>
                  ...
                  {topLocations.map((location) => {
                    ...
                    return (
                      <div key={location.id} className="flex items-center justify-between">
                        <div>
                          <span className="font-medium text-earth-800">{location.name}</span>
                          {location.region && (
                            <span className="text-sm text-earth-600 ml-2">({location.region})</span>
                          )}
                        </div>
                        <span className="text-sm text-earth-600">{totalCount} items</span>
                      </div>
